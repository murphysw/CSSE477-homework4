...
//				
				...
				addMethod(request.getMethod().toString().toUpperCase());
				if(checkForMethodOverload(request.getMethod().toString().toUpperCase())){
					response = HttpResponseFactory.create503ServiceUnavaliable(Protocol.CLOSE);
				}
...
<<<<<<< HEAD
	...
	private boolean checkForMethodOverload(String method) {
		int overloadAmount = 0;
